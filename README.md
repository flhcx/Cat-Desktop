...
| 📦 **快捷方式管理** | 支持拖拽 `.lnk`、`.exe`、`.url` 文件到面板中统一管理 |
| 🚀 **一键启动** | 左键单击图标即可启动对应程序，自动处理管理员权限 |
| 🖱️ **自由拖拽** | 窗口全区域可拖拽移动，自动限制屏幕边界防止丢失 |
| 🔲 **窗口可伸缩** | 展开状态下支持鼠标边缘拖拽调整窗口大小（8 个方向） |
| 📂 **一键自动归类** | 智能识别快捷方式类型，按 11 个预设分类自动分组显示 |
| 🔍 **一键扫描桌面** | 扫描桌面上的快捷方式并自动归类导入，智能去重 |
| 🙈 **隐藏桌面图标** | 一键隐藏/显示桌面图标，兼容 Wallpaper Engine 动态壁纸 |
| ⚙️ **设置面板** | 内置设置页：记住位置、开机自启、自定义悬浮球图片、展开特效 |
...
| 🎨 **精美 UI** | 无边框透明窗口、毛玻璃效果、6 种丝滑展开/折叠动画特效 |
...
| 📏 悬浮球大小 | 自定义调整悬浮球尺寸 |
| 🎬 展开特效 | 6 种展开动画可选（缩放/下滑/右滑/弹性/旋转/淡入） |
| 🙈 隐藏桌面图标 | 隐藏桌面所有图标，兼容 Wallpaper Engine |
| 🔍 扫描桌面图标 | 一键扫描桌面快捷方式并自动归类导入 |
| 📂 一键自动归类 | 智能识别已有快捷方式并按类别分组 |

## 🏗️ 项目结构

```
Cat-Desktop/
├── App.xaml / App.xaml.cs        # 应用入口（含单实例检测）
...
│   ├── ShortcutCategorizer.cs    # 智能分类引擎 & 桌面扫描
...
- **桌面图标控制**：Win32 API（SendMessage + WM_COMMAND）
- **单实例检测**：System.Threading.Mutex
...
### v0.3 (Pre-release)
- 🙈 新增隐藏桌面图标功能（兼容 Wallpaper Engine 动态壁纸）
- 📂 新增一键自动归类（11个预设分类：浏览器/开发工具/游戏/社交通讯等）
- 🔍 新增一键扫描桌面快捷方式并自动归类导入
- 🔲 窗口支持鼠标边缘拖拽调整大小（8个方向）
- 🔒 新增单实例运行检测（防止重复打开）
- 🎬 新增 6 种展开动画特效可选
- 📏 新增悬浮球大小自定义滑块
- 🎨 美化 Slider 滑块、ScrollBar 滚动条样式
- 🐛 修复切换展开特效时程序卡死的动画竞态 bug
